...
the 2-covering group $F/R^2[F,R]$ of $H$ (ANUPQ). All $7\,111\,878$ groups of order 512 with $d(H)=5$ and all
groups of order $\le 256$ with the relevant rank were excluded. For $r=2$ and orders $\ge 2^{12}$ the agent
relies on the classification of 2-groups with exactly three involutions (Konvisser, Janko), which it could not
consult.
...
\end{theorem}
The proof is a perfect matching in a connected vertex-transitive graph on the coset space $H\backslash K$, which
exists by the Gallai--Edmonds structure theorem. The second construction takes $A=HX$ (a union of right cosets of
...
dancing-links search. In this way 267 of the 400 sizes were realised by explicit sets, every one re-verified in
...
$G/L\cong A_4$?
\end{problem}
The agent showed that in any example $I=K\cap L$ satisfies $K/I\cong V_4$, $L/I\cong C_4$, $G/I\cong C_4\times A_4$,
...
decompositions); computationally, no group of order $\le 2000$ is an example, and no group of order $\le 256$, of
order $512$ and rank $\le 4$, or of order $260$--$1000$ divisible by 4 (except 512 of rank $\ge 5$ and 768)
...
$\star$-commutators, with $ab$ of order 2; hence it violates the hypothesis. Consequently a counterexample to
19.56 must be a minimal non-soluble group with non-trivial Frattini subgroup (and every non-soluble group whose
minimal non-soluble subgroups have trivial Frattini subgroup satisfies 19.56).
\end{theorem}
The proof is uniform over Thompson's list: involutions are $\star$-commutators via $A_4$ or a Borel subgroup,
elements of odd prime order via a dihedral subgroup ($[b,t]=b^{-2}$). All 208 non-soluble groups of order
$\le 2000$ and all 443 perfect groups of order $\le 10^5$ (including many with large Frattini subgroups) violate the
...
\end{problem}
The answer is yes when $|G|$ has two prime divisors (or more generally $G=PQ$ for two Sylow subgroups): if
...
equal $O_p(G)$, as predicted by Zenkov's theorem, and a common $x$ was found within a few random samples.

\subsection{Problem 21.111 (D.~O.~Revin, Zh.~Yang): $\tau$-groups}
A non-identity automorphism $x$ of a simple group $S$ is a $\tau$-automorphism if any two conjugates of $x$ in
