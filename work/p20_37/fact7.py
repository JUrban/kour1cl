...
solname = fname.replace('caseb_', 'solb_')  # shared
...
def attempt(a, h, sg, kk, sgk, trials):
    ...
    if m <= 80: trials = 300000
    elif m <= 200: trials = 40000
    elif m <= 600: trials = 3000
    else: trials = 100
    lim = 100000 if m <= 200 else nodelimit
    for trial in range(trials):
        ...
        r = subprocess.run(["./dlx", str(lim), str(random.randrange(1 << 30))], input=inp, capture_output=True, text=True, timeout=7200)
        line = r.stdout.strip()
