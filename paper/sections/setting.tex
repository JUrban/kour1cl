...
15\,364 lines). The 21st edition lists the unsolved problems of all issues 1--21 (solved problems are moved to a
separate archive), so every problem in the file was open when the edition was compiled; new problems of the 21st
issue occupy roughly lines 8000--9100.
...
\begin{itemize}
\item GAP~4.16.1 with its standard package distribution, in particular the SmallGroups library (all groups of order
...
package.
\item nauty~2.8.6 (\code{geng}, \code{nauty.a}), built from the copy shipped with GAP's Grape package, for graph
...
harness replaces the older part by a summary. One such compaction happened during the run (after about seven
hours); the agent continued from the summary and from the files on disk.
